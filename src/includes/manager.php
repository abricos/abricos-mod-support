...
class SupportManager extends Ab_ModuleManager {
    ...
    private function _AJAX($d){
        switch ($d->do){
            case 'messagesave':
                return $this->MessageSave($d->message);
            case 'message':
                return $this->Message($d->messageid);
            case 'sync':
                return $this->Sync();
            case 'messageclose':
                return $this->MessageClose($d->messageid);
            case 'messageremove':
                return $this->MessageRemove($d->messageid);
        }
        return null;
    }
    ...
    public function IsCommentAppend($contentid){
        $message = SupportQuery::MessageByContentId($this->db, $contentid, true);
        if (!$this->MessageAccess($message)){
            return false;
        }
        if ($message['st'] == SupportStatus::CLOSED || $message['st'] == SupportStatus::REMOVED){
            return false;
        }
    ...
    public function MessageClose($messageid){
        ...
        if ($msg['st'] != SupportStatus::OPENED){
            return null;
        } // закрыть можно только открытое сообщение
